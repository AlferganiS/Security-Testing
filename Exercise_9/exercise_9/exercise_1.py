...
class tany(object):
    ...
    def create(self, x: Any):
        ...
        tan = tany(x)
        tan.taint = self.taint
        return tan

    
    @staticmethod
    def make_wrapper(fun_container_op: str):
        """Make `fun_container_op` from any class a method in `tany`"""
        def proxy(self, *args, **kwargs):
            ...
            try:
                res = getattr(self.value, fun_container_op)(*args, **kwargs)
            except AttributeError:
                res = getattr(args[0], fun_container_op.replace('__r', '__'))(self.value, *args[1:], **kwargs) 
            return self.create(res)

        return proxy
    
    @staticmethod
    def make_wrapper_unchanged_return(fun_container_op: str):
        """Make `fun_container_op` from any class a method in `tany` but returns a not tainted object"""
        def proxy(self, *args, **kwargs):
            args = list(args)
            for i in range(len(args)):
                if isinstance(args[i], tany):
...
for rop in rops:
    setattr(tany, rop, tany.make_wrapper(rop))

for num_op in number_ops:
    setattr(tany, num_op, tany.make_wrapper(num_op)) 

for str_op in str_ops:
    setattr(tany, str_op, tany.make_wrapper(str_op))  

for container_op in container_ops:
    setattr(tany, container_op, tany.make_wrapper(container_op)) 

for container_op_unchanged in container_ops_unchanged:
    setattr(tany, container_op_unchanged, tany.make_wrapper_unchanged_return(container_op_unchanged))