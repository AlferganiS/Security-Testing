...
def parse_and_process(args):
    ...
    # Create a mutually exclusive group for the preprocessing flags
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--identity', action='store_const', const='identity', dest='preprocessing', help='Do not change individual strings before concatenating.')
    group.add_argument('--sort', action='store_const', const='sort', dest='preprocessing', help='Sort individual strings in normal order before concatenating.')
    group.add_argument('--reversesort', action='store_const', const='reversesort', dest='preprocessing', help='Sort individual strings in reverse order before concatenating.')

    # Add the optional flag for outputting average string length
    parser.add_argument('--output-average-string-length', action='store_true', help='Output the average string length.')
